...
### SPA localization (i18n)
The React SPA is fully localized with **react-i18next** + **i18next-browser-languagedetector** (`OmniCard.Web/ClientApp/src/i18n/`). `index.ts` inits a single `translation` bundle assembled by spreading one JSON file per feature area from `locales/en-US/` (namespaces: `common`, `nav`, `auth`, `dashboard`, `collection`, `locations`, `binder`, `sets`, `scan`, `sales`, `inventory`, `importing`, `lists`, `trades`, `settings`, `deckbox`, `dialogs`, `search`) — each file is keyed by its namespace object so the spreads never collide. Language follows the browser culture (detection order: `?lng=` querystring → `navigator` → `<html lang>`) with **`fallbackLng: 'en-US'`** (en-US is the only bundle shipped, and the source of truth). Imported once from `src/main.tsx`. **Conventions when touching the SPA:** never hard-code user-facing text — `const { t } = useTranslation();` then `t('<ns>.<key>')`, reusing `common.*` for generic terms; format every number/currency/date via `useFormatters()` from `src/i18n/format.ts` (`fmt.money/number/percent/date/dateTime`), never `toLocaleString`/`toFixed`/`new Date().toLocale*` directly — formatting follows the browser locale while currency stays USD (only presentation localized); don't translate server data (card/set/customer names, error messages) or enum identifiers sent to the API. Add a language by copying `locales/en-US/` to `locales/<culture>/`, translating the values, and registering it in `index.ts` — no component changes.
...
- **Localization:** any new/changed SPA UI must add its strings to the appropriate `OmniCard.Web/ClientApp/src/i18n/locales/en-US/*.json` namespace and go through `t()` + `useFormatters()` (see [SPA localization](#spa-localization-i18n)) — don't reintroduce hard-coded text or raw `toLocaleString`/`toFixed` display formatting. Keep the README's Localization section current if the namespace set or detection/fallback behavior changes.
